...
const REL_TYPES = [
  { value: '1:1', label: 'Uno a Uno' },
  { value: '1:N', label: 'Uno a Molti' },
  { value: 'N:N', label: 'Molti a Molti' },
];
...
export default function Sidebar() {
  ...
  if (!selected) {
    return (
      <aside className="sidebar">
        <div className="sidebar-title">Proprietà</div>
        <div className="sidebar-content">
          <div style={{color:'#888'}}>Seleziona una tabella o relazione</div>
        </div>
      </aside>
    );
  }
  if (selected.type === 'relation' && relation) {
    return (
      <aside className="sidebar">
        <div className="sidebar-title">Relazione</div>
        <div className="sidebar-content">
          <div style={{marginBottom:12}}>
            <b>Colonne collegate:</b><br/>
          ...
          <button className="sidebar-btn" onClick={() => handleDeleteRelation(relation.id)} style={{color:'#ef4444',marginTop:12}}>Elimina relazione</button>
        </div>
      </aside>
    );
  }
  if (selected.type === 'table' && table) {
    ...
    return (
      <aside className="sidebar">
        <div className="sidebar-title">Tabella: <input className="sidebar-input" value={table.name} onChange={e => handleTableNameChange(table.id, e.target.value)} style={{fontWeight:'bold',fontSize:'1.1em',border:'none',background:'transparent',outline:'none'}} /></div>
        <div className="sidebar-content">
          <div style={{fontWeight:'bold',marginBottom:8}}>Colonne</div>
          {table.columns.map((col, idx) => (
            <div key={idx} style={{display:'flex',alignItems:'center',gap:6,marginBottom:3,fontSize:'0.97em'}}>
              ...
              <button className="sidebar-btn" onClick={() => handleRemoveColumn(table.id, idx)} title="Rimuovi colonna" style={{color:'#ef4444',border:'none',background:'none',fontSize:'1.1em',cursor:'pointer'}}>✕</button>
            </div>
          ))}
          <button className="sidebar-btn" onClick={() => handleAddColumn(table.id)} style={{marginTop:8}}>+ Colonna</button>
          {/* Pulsante per aggiungere relazione */}
          <div style={{marginTop:18}}>
            <button className="sidebar-btn" onClick={handleStartRel} style={{background:'#2563eb',color:'#fff',fontWeight:'bold'}}>+ Aggiungi relazione</button>
          </div>
          {/* Stepper creazione relazione */}
          {relStep > 0 && (
            <div style={{marginTop:14,background:'#f3f4f6',padding:10,borderRadius:8}}>
              <div style={{fontWeight:'bold',marginBottom:6}}>Nuova relazione</div>
              {relStep === 1 && (
                <>
                  <div>1. Seleziona tabella di destinazione:</div>
                  ...
                  <div style={{marginTop:8}}><button className="sidebar-btn" onClick={resetRel}>Annulla</button></div>
                </>
              )}
              {relStep === 2 && (
                <>
                  <div>2. Seleziona colonna sorgente:</div>
                  <select className="sidebar-select" value={fromColIdx||''} onChange={e => handleSelectFromCol(Number(e.target.value))}>
                    <option value="">-- Scegli colonna --</option>
                    {table.columns.map((col,idx)=>(
              ...
              {relStep === 3 && (
                <>
                  <div>3. Seleziona colonna di destinazione:</div>
              ...
              {relStep === 4 && (
                <>
                  ...
                  <button className="sidebar-btn" onClick={handleConfirmRel} style={{background:'#2563eb',color:'#fff',fontWeight:'bold'}}>Crea relazione</button>
                  <button className="sidebar-btn" onClick={resetRel} style={{marginLeft:8}}>Annulla</button>
                </>
              )}
            </div>
          )}
